...
# 通用消息处理函数
async def handle_ai_response_with_tts(
    websocket: WebSocket,
    manager: ConnectionManager,
    chat_history: ChatHistory,
    session_id: str,
        ...
        while True:
            ...
            if message_type == "text":
                ...
                if user_message:
                    ...
                    await handle_ai_response_with_tts(
                        websocket=websocket,
                        manager=manager,
                ...
                if audio_base64:
                    ...
                    if transcribed_text and len(transcribed_text.strip()) > 1:
                        ...
                        # 使用通用函数处理AI响应和TTS
                        tts_engine = data.get("tts_engine", "gtts")
                        full_response = await handle_ai_response_with_tts(
                            websocket=websocket,
                            manager=manager,
                            chat_history=chat_history,
                            session_id=session_id,
